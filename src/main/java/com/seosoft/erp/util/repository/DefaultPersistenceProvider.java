package com.seosoft.erp.util.repository;
...
/**
 * 
 * @author Boris lam This class is use when you use DefaultRepositoryFactory to override default repository factory class.
 * 
 */
...
public enum DefaultPersistenceProvider implements QueryExtractor, Serializable {
	...
	HIBERNATE("org.hibernate.ejb.HibernateEntityManager") {
		...
		/**
		 * Return custom placeholder ({@code *}) as Hibernate does create invalid queries for count queries for objects with compound keys.
		 * 
		 * @see HHH-4044
		 * @see HHH-3096
		 */
	...
	/**
	 * Determines the {@link PersistenceProvider} from the given {@link EntityManager}. If no special one can be determined
